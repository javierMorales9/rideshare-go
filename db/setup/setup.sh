...
if [ -z "$RIDESHARE_DB_PASSWORD" ]; then
    ...
    echo "OR generate a new value (See comments in: db/setup/setup.sh)"
    exit 1
fi
...
if [ -z "$DB_URL" ]; then
    ...
    echo
    echo "See: db/setup/setup.sh"
    echo "Run: export DB_URL='postgres://postgres:@localhost:5432/postgres'"
    exit 1
fi

# Set up Roles and Users on your PostgreSQL instance
psql $DB_URL -v password_to_save=$RIDESHARE_DB_PASSWORD -a -f db/setup/create_role_owner.sql
psql $DB_URL -a -f db/setup/create_role_readwrite_users.sql
psql $DB_URL -a -f db/setup/create_role_readonly_users.sql
psql $DB_URL -v password_to_save=$RIDESHARE_DB_PASSWORD -a -f db/setup/create_role_app_user.sql
psql $DB_URL -v password_to_save=$RIDESHARE_DB_PASSWORD -a -f db/setup/create_role_app_readonly.sql

# Set up Rideshare development database
psql $DB_URL -a -f db/setup/create_database.sql

# Revoke database privileges on public, drop public schema
psql $DB_URL -a -f db/setup/revoke_drop_public_schema.sql

# Create rideshare schema
psql $DB_URL -a -f db/setup/create_schema.sql

# Perform GRANT operations
psql $DB_URL -a -f db/setup/create_grants_database.sql
psql $DB_URL -a -f db/setup/create_grants_schema.sql

# Alter the default privileges
psql $DB_URL -a -f db/setup/alter_default_privileges_readwrite.sql
psql $DB_URL -a -f db/setup/alter_default_privileges_readonly.sql
psql $DB_URL -a -f db/setup/alter_default_privileges_public.sql
...
echo
echo "If you ran as: 'sh db/setup/setup.sh 2>&1 | tee -a output.log'"
